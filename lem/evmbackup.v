...
Definition empty_storage   (_ : word256)   : word256 := word256FromNumeral 0.
...
Definition Gextcode  (blocknumber : nat )  : Z := 
  if nat_gteb blocknumber( 2463*1000) then(Zpred (Zpos (P_of_succ_nat 700))) else(Zpred (Zpos (P_of_succ_nat 20))).
(* [?]: removed value specification. *)

Definition Gbalance  (blocknumber : nat )  : Z := 
  if nat_gteb blocknumber( 2463*1000) then(Zpred (Zpos (P_of_succ_nat 400))) else(Zpred (Zpos (P_of_succ_nat 20))).
(* [?]: removed value specification. *)

Definition Gsload  (blocknumber : nat )  : Z := 
  if nat_gteb blocknumber( 2463*1000) then(Zpred (Zpos (P_of_succ_nat 200))) else(Zpred (Zpos (P_of_succ_nat 50))).
(* [?]: removed value specification. *)
...
Definition Gsuicide  (blocknumber : nat )  : Z := 
  if nat_gteb blocknumber( 2463*1000) then(Zpred (Zpos (P_of_succ_nat 5000))) else(Zpred (Zpos (P_of_succ_nat 0))).
(* [?]: removed value specification. *)
...
Definition Gcall  (blocknumber : nat )  : Z := 
  if nat_gteb blocknumber( 2463*1000) then(Zpred (Zpos (P_of_succ_nat 700))) else(Zpred (Zpos (P_of_succ_nat 40))).
(* [?]: removed value specification. *)
...
(*
Program Fixpoint cut_memory  (idx : word256 ) (n : word256 ) (memory1 : word256  -> word8 )  : list (word8 ):= 
  if classical_boolean_equivalence n(word256FromNumeral 0) then []
...
Definition call  (v : variable_ctx ) (c : constant_ctx )  : instruction_result :=  match ((vctx_stack v)) with 
 | e0 :: e1 :: e2 :: e3 :: e4 :: e5 :: e6 :: rest =>
     strict_if (word256ULT ((vctx_balance v)(cctx_this c)) e2)
...
Definition delegatecall  (v : variable_ctx ) (c : constant_ctx )  : instruction_result :=  match ((vctx_stack v)) with 
 | e0 :: e1 :: e3 :: e4 :: e5 :: e6 :: rest =>
    if word256ULT ((vctx_balance v)(cctx_this c))(vctx_value_sent v) then
        (InstructionContinue
           (vctx_advance_pc c
             {[ v with vctx_stack := (word256FromNumeral 0 :: rest)
              ]}
...
Definition callcode  (v : variable_ctx ) (c : constant_ctx )  : instruction_result :=  match ((vctx_stack v)) with 
 | e0 :: e1 :: e2 :: e3 :: e4 :: e5 :: e6 :: rest =>
    if word256ULT ((vctx_balance v)(cctx_this c)) e2 then
        (InstructionContinue
           (vctx_advance_pc c
             {[ v with vctx_stack := (word256FromNumeral 0 :: rest)
              ]}
...
Definition create  (v : variable_ctx ) (c : constant_ctx )  : instruction_result :=  match ((vctx_stack v)) with 
  | vl :: code_start :: code_len :: rest =>
      if word256ULT ((vctx_balance v)(cctx_this c)) vl then
        (InstructionContinue
           (vctx_advance_pc c
             {[ v with vctx_stack := (word256FromNumeral 0 :: rest)
              ]}
...
Definition suicide  (v : variable_ctx ) (c : constant_ctx )  : instruction_result :=  match ((vctx_stack v)) with 
 | dst :: _ =>
     let new_balance := (fun (addr : word160 ) =>
        ...
        if classical_boolean_equivalence addr (w256_to_address dst) then word256Add ((vctx_balance v)(cctx_this c)) ((vctx_balance v) addr) else(vctx_balance
        v) addr) in
...
(Gsuicide blocknumber) (if recipient_empty && nat_gteb blocknumber( 2463*1000) then Gnewaccount else(Zpred (Zpos (P_of_succ_nat 0)))).
(* [?]: removed value specification. *)
...
Program Fixpoint log256floor  (x : Z )  : Z := x.
(* [?]: removed value specification. *)
...
Definition predict_gas  (i : inst ) (v : variable_ctx ) (c : constant_ctx )  : Z := 
  C(vctx_memory_usage v) (new_memory_consumption i v) (vctx_next_instruction_default v c)
    ...
    (negb ((vctx_account_existence v) (vctx_recipient v c))) ((vctx_storage v) (vctx_stack_default((Zpred (Zpos (P_of_succ_nat 0)))) v))
...
Program Fixpoint word_exp  (i : Z ) (n : nat )  : Z := 
  ...
  | _ =>
      let half := Coq.Numbers.Natural.Peano.NPeano.div n( 2) in
      let modulo :=  ( Coq.Numbers.Natural.Peano.NPeano.modulo n( 2)) in
      let recur := word_exp i half in Coq.ZArith.Zdiv.Zmod
         ( Coq.ZArith.BinInt.Zmult (Coq.ZArith.BinInt.Zmult( Coq.ZArith.Zpower.Zpower_nat i modulo) recur) recur) ( Coq.ZArith.Zpower.Zpower_nat((Zpred (Zpos (P_of_succ_nat 2))))( 256))
  end.*)
(* [?]: removed value specification. *)
...
Definition instruction_sem  (v : variable_ctx ) (c : constant_ctx ) (inst1 : inst )  : instruction_result := 
  ...
  | Arith EXP => stack_2_1_op v c (fun (a : word256 ) (exponent : word256 ) =>
      word256FromInteger
       (word_exp (uint a) (word256ToNatural exponent)))
  ...
  | Info EXTCODESIZE => stack_1_1_op v c
       (fun (arg : word256 ) => word256FromInteger(program_length ((vctx_ext_program v) (w256_to_address arg))))
...
Program Fixpoint put_return_values  (orig : word256  -> word8 ) (lst : list (word8 )) (b : Z ) (s : Z )  : word256  -> word8 := 
  if int_lteb s((Zpred (Zpos (P_of_succ_nat 0)))) then orig else
  match ( lst) with 
  | [] => orig
  | h :: t => put_return_values (fun (addr : word256 ) => if classical_boolean_equivalence addr (word256FromInteger b) then h else orig addr) t ( Coq.ZArith.BinInt.Zplus b((Zpred (Zpos (P_of_succ_nat 1))))) ( Coq.ZArith.BinInt.Zminus s((Zpred (Zpos (P_of_succ_nat 1)))))
  end.
