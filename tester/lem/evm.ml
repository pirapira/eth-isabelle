...
let rec cut_memory idx n memory1:(word8)list=  
 (if n =(word256FromNumeral 0) then []
  else memory1 idx :: cut_memory ( word256Add idx(word256FromNumeral 1)) ( word256Minus n(word256FromNumeral 1)) memory1)
...
let rec put_return_values orig lst b s:word256 ->word8=  
 (if Nat_big_num.less_equal s(Nat_big_num.of_int 0) then orig else
  (match lst with
  | [] -> orig
  | h :: t -> put_return_values (fun addr -> if addr = word256FromInteger b then h else orig addr) t ( Nat_big_num.add b(Nat_big_num.of_int 1)) ( Nat_big_num.sub s(Nat_big_num.of_int 1))
  ))
...
let dup_inst_numbers m1:int*int=  ((word4ToUInt m1 + 1), (word4ToUInt m1 + 2))
...
((word4ToUInt m1 + 2), (word4ToUInt m1 + 2))
...
(*val magic_block : natural*)

(*val Gextcode : natural -> integer*)
let gextcode blocknumber:Nat_big_num.num=  
 (if Nat_big_num.greater_equal blocknumber (Nat_big_num.mul(Nat_big_num.of_int 2463)(Nat_big_num.of_int 1000)) then Nat_big_num.of_int 700 else Nat_big_num.of_int 20)

(*val Gbalance : natural -> integer*)
let gbalance blocknumber:Nat_big_num.num=  
 (if Nat_big_num.greater_equal blocknumber (Nat_big_num.mul(Nat_big_num.of_int 2463)(Nat_big_num.of_int 1000)) then Nat_big_num.of_int 400 else Nat_big_num.of_int 20)

(*val Gsload : natural -> integer*)
let gsload blocknumber:Nat_big_num.num=  
 (if Nat_big_num.greater_equal blocknumber (Nat_big_num.mul(Nat_big_num.of_int 2463)(Nat_big_num.of_int 1000)) then Nat_big_num.of_int 200 else Nat_big_num.of_int 50)
...
let gsuicide blocknumber:Nat_big_num.num=  
 (if Nat_big_num.greater_equal blocknumber (Nat_big_num.mul(Nat_big_num.of_int 2463)(Nat_big_num.of_int 1000)) then Nat_big_num.of_int 5000 else Nat_big_num.of_int 0)
...
let gcall blocknumber:Nat_big_num.num=  
 (if Nat_big_num.greater_equal blocknumber (Nat_big_num.mul(Nat_big_num.of_int 2463)(Nat_big_num.of_int 1000)) then Nat_big_num.of_int 700 else Nat_big_num.of_int 40)
...
(*val Gexpbyte : integer*)
let gexpbyte:Nat_big_num.num= (Nat_big_num.of_int 50)
...
type program = {
  ...
  program_annotation : Nat_big_num.num -> annotation list; (* a mapping from positions to annotations *)
}
...
(* Annotations are stored in a mapping that maps positions into lists of annotations. *)
...
(*val prepend_annotation : integer -> annotation -> (integer -> list annotation) -> (integer -> list annotation)*)
let prepend_annotation pos annot orig p:(aenv ->bool)list=  (if Nat_big_num.equal pos p then annot :: orig p else orig p)

(* Currently annotations are inserted into a mapping with Isabelle/HOL's mapping updates. *)
(* When this causes performance problems, I need to switch to AVL trees again. *)

(*val program_annotation_of_lst : int -> list inst -> integer -> list annotation*)
let rec program_annotation_of_lst pos lst:Nat_big_num.num ->(aenv ->bool)list=  ((match lst with
| [] -> (fun _ -> [])
...
(* For interpreting the annotations, I first need to construct the annotation environment *)
(* out of the current execution environments.  When I try to remove this step, I face some *)
(* circular definitions of data types. *)

(*val build_aenv : variable_ctx -> constant_ctx -> aenv*)
let build_aenv v c1:aenv=  ({
  aenv_stack = (v.vctx_stack);
  aenv_memory = (v.vctx_memory);
  aenv_storage = (v.vctx_storage);
  aenv_balance = (v.vctx_balance);
  aenv_caller = (v.vctx_caller);
  aenv_value_sent = (v.vctx_value_sent);
  aenv_data_sent = (v.vctx_data_sent);
  aenv_storage_at_call = (v.vctx_storage_at_call);
  aenv_balance_at_call = (v.vctx_balance_at_call);
  aenv_this = (c1.cctx_this);
  aenv_origin = (v.vctx_origin);
})
...
let call v c1:instruction_result=  ((match v.vctx_stack with
 | e0 :: e1 :: e2 :: e3 :: e4 :: e5 :: e6 :: rest ->
     strict_if (word256ULT (v.vctx_balance c1.cctx_this) e2)
        (fun _ -> (InstructionContinue
           (vctx_advance_pc c1
             { v with
                vctx_stack = ((word256FromNumeral 0) :: rest)
             }
           )))
       (fun _ -> InstructionToEnvironment( (ContractCall
         { callarg_gas = e0;
        ...
        ({ (vctx_advance_pc c1 v) with
           vctx_stack = rest;
           vctx_balance = (update_balance c1.cctx_this (fun orig -> word256Minus orig e2) v.vctx_balance);
           vctx_memory_usage = (m (m v.vctx_memory_usage e3 e4) e5 e6) }),
...
let delegatecall v c1:instruction_result=  ((match v.vctx_stack with
 | e0 :: e1 :: e3 :: e4 :: e5 :: e6 :: rest ->
    if word256ULT (v.vctx_balance c1.cctx_this) v.vctx_value_sent then
        (InstructionContinue
           (vctx_advance_pc c1
             { v with
                vctx_stack = ((word256FromNumeral 0) :: rest)
       ...
       InstructionToEnvironment(
         (ContractDelegateCall
           { callarg_gas = e0;
              ...
              callarg_output_size = e6 }),
          ({ (vctx_advance_pc c1 v) with
                 vctx_stack = rest; vctx_memory_usage = (m (m v.vctx_memory_usage e3 e4) e5 e6) }),
...
let callcode v c1:instruction_result=  ((match v.vctx_stack with
 | e0 :: e1 :: e2 :: e3 :: e4 :: e5 :: e6 :: rest ->
    if word256ULT (v.vctx_balance  c1.cctx_this) e2 then
        (InstructionContinue
           (vctx_advance_pc c1
             { v with
                vctx_stack = ((word256FromNumeral 0) :: rest)
...
let create v c1:instruction_result=  ((match v.vctx_stack with
  | vl :: code_start :: code_len :: rest ->
      if word256ULT (v.vctx_balance c1.cctx_this) vl then
        (InstructionContinue
           (vctx_advance_pc c1
             { v with
                vctx_stack = ((word256FromNumeral 0) :: rest)
        ...
        let new_balance = (update_balance c1.cctx_this (fun orig -> word256Minus orig vl) v.vctx_balance) in
        InstructionToEnvironment(
           (ContractCreate
             { createarg_value = vl; createarg_code = code }),
            ({ (vctx_advance_pc c1 v) with vctx_stack = rest;
                 vctx_balance = new_balance;
                 vctx_memory_usage = (m v.vctx_memory_usage code_start code_len) }),
...
let ret v _:instruction_result=   ((match v.vctx_stack with
 | e0 :: e1 :: _ ->
     let new_v = ({ v with vctx_memory_usage = (m v.vctx_memory_usage e0 e1) }) in
  ...
  | pos :: vl :: rest ->
       ...
       InstructionContinue (vctx_advance_pc c1
         { v with vctx_stack = rest; vctx_memory = new_memory;
           vctx_memory_usage = (m v.vctx_memory_usage pos(word256FromNumeral 32))
          })
  | _ -> instruction_failure_result v [TooShortStack]
))
...
let mload v c1:instruction_result=  ((match v.vctx_stack with
 | pos :: rest ->
      ...
      InstructionContinue
        (vctx_advance_pc c1 { v with vctx_stack = (value :: rest); vctx_memory_usage = (m v.vctx_memory_usage pos(word256FromNumeral 32)) })
 | _ -> instruction_failure_result v [TooShortStack]
))
...
let mstore8 v c1:instruction_result=  ((match v.vctx_stack with
 | pos :: vl :: rest ->
   ...
   InstructionContinue (vctx_advance_pc c1
          { v with vctx_stack = rest; vctx_memory_usage = (m v.vctx_memory_usage pos(word256FromNumeral 8));
...
let calldatacopy v c1:instruction_result=  ((match v.vctx_stack with
 | dst_start :: src_start :: len :: rest ->
       ...
       InstructionContinue (vctx_advance_pc c1
         { v with vctx_stack = rest; vctx_memory = new_memory;
            vctx_memory_usage = (m v.vctx_memory_usage dst_start len) })
 | _ -> instruction_failure_result v [TooShortStack]
))
 ...
 (let new_log_entry = (create_log_entry n v c1) in
   InstructionContinue
    (vctx_advance_pc c1 (vctx_pop_stack (n+ 2)
       { v with
          vctx_memory_usage = (m v.vctx_memory_usage (vctx_stack_default( 0) v) (vctx_stack_default( 1) v));
          vctx_logs = (new_log_entry ::  v.vctx_logs)
        })))
...
let sha3 v c1:instruction_result=  ((match v.vctx_stack with
 | start :: len :: rest ->
      InstructionContinue (
        vctx_advance_pc c1
           { v with vctx_stack = (keccak (cut_memory start len v.vctx_memory) :: rest);
                     vctx_memory_usage = (m v.vctx_memory_usage start len) })
 | _ -> instruction_failure_result v [TooShortStack]
))
...
let suicide v c1:instruction_result=  ((match v.vctx_stack with
 ...
 (if addr = c1.cctx_this then(word256FromNumeral 0) else
        if addr = w256_to_address dst then word256Add (v.vctx_balance c1.cctx_this) (v.vctx_balance addr) else
        v.vctx_balance addr) in
...
let csuicide recipient_empty blocknumber:Nat_big_num.num=  (Nat_big_num.add  
(gsuicide blocknumber) (if recipient_empty && Nat_big_num.greater_equal blocknumber (Nat_big_num.mul(Nat_big_num.of_int 2463)(Nat_big_num.of_int 1000)) then gnewaccount else Nat_big_num.of_int 0))
...
let cnew value emp:Nat_big_num.num=  (if emp && not (value =(word256FromNumeral 0)) then gnewaccount else Nat_big_num.of_int 0)
...
let cextra value emp blocknumber:Nat_big_num.num=  (Nat_big_num.add (Nat_big_num.add (gcall blocknumber) (cxfer value)) (cnew value emp))
...
(*val Cgascap : w256 -> w256 -> bool -> integer -> natural -> integer*)
let cgascap mu0 mu1 emp remaining_gas blocknumber:Nat_big_num.num=  
 (if Nat_big_num.greater_equal remaining_gas (cextra mu1 emp blocknumber) then
    Nat_big_num.min (l ( Nat_big_num.sub remaining_gas (cextra mu1 emp blocknumber))) (uint mu0)
  else
    uint mu0)
...
(* However, it takes the three topmost stack elements,
   a boolean indicating emptiness of the recipient, and the remaining gas of the caller *)
(*val Ccallgas : w256 -> w256 -> w256 -> bool -> integer -> natural -> integer*)
let ccallgas mu0 mu1 mu2 emp remaining_gas blocknumber:Nat_big_num.num=  
 (if mu2 =(word256FromNumeral 0) then
...
(*val Ccall : w256 -> w256 -> w256 -> bool -> integer -> natural -> integer*)
let ccall mu0 mu1 mu2 emp remaining_gas blocknumber:Nat_big_num.num=  (Nat_big_num.add (cgascap mu0 mu1 emp remaining_gas blocknumber) (cextra mu2 emp blocknumber))
...
(*val thirdComponentOfC : inst -> w256 -> w256 -> w256 -> w256 -> bool -> w256 -> w256 -> integer -> natural -> integer*)
let thirdComponentOfC i s0 s1 s2 s3 recipient_empty orig_val new_val remaining_gas blocknumber:Nat_big_num.num=  
 ((match i with
  | Storage SSTORE -> csstore orig_val new_val
  | Arith EXP -> Nat_big_num.add gexp (if s1 =(word256FromNumeral 0) then Nat_big_num.of_int 0 else Nat_big_num.mul gexpbyte ( Nat_big_num.add(Nat_big_num.of_int 1) (log256floor (uint s1))))
  ...
  | Memory EXTCODECOPY -> Nat_big_num.add (gextcode blocknumber) (Nat_big_num.mul gcopy ( Nat_big_num.div( Nat_big_num.add(uint s3)(Nat_big_num.of_int 31))(Nat_big_num.of_int 32)))
...
let c old_memory_consumption new_memory_consumption1 i s0 s1 s2 s3 recipient_empty orig new_val remaining_gas blocknumber:Nat_big_num.num=  (Nat_big_num.add  (Nat_big_num.sub  
...
(*val new_memory_consumption : inst -> variable_ctx -> integer*)
let new_memory_consumption i v:Nat_big_num.num=  
 ((match i with
  | Arith SHA3 -> m v.vctx_memory_usage (vctx_stack_default( 0) v) (vctx_stack_default( 1) v)
  | Memory CALLDATACOPY ->
    m v.vctx_memory_usage (vctx_stack_default( 0) v) (vctx_stack_default( 2) v)
  | Memory CODECOPY ->
    m v.vctx_memory_usage (vctx_stack_default( 0) v) (vctx_stack_default( 2) v)
  | Memory EXTCODECOPY ->
    m v.vctx_memory_usage (vctx_stack_default( 1) v) (vctx_stack_default( 3) v)
  | Memory MLOAD ->
    m v.vctx_memory_usage (vctx_stack_default( 0) v)(word256FromNumeral 32)
  | Memory MSTORE ->
    m v.vctx_memory_usage (vctx_stack_default( 0) v)(word256FromNumeral 32)
  | Memory MSTORE8 ->
    m v.vctx_memory_usage (vctx_stack_default( 0) v)(word256FromNumeral 1)
  | Misc CREATE ->
    m v.vctx_memory_usage (vctx_stack_default( 1) v) (vctx_stack_default( 2) v)
  | Misc CALL ->
    m (m v.vctx_memory_usage (vctx_stack_default( 3) v) (vctx_stack_default( 4) v))
      (vctx_stack_default( 5) v) (vctx_stack_default( 6) v)
  | Misc CALLCODE ->
    m (m v.vctx_memory_usage (vctx_stack_default( 3) v) (vctx_stack_default( 4) v))
      (vctx_stack_default( 5) v) (vctx_stack_default( 6) v)
  | Misc DELEGATECALL ->
    m (m v.vctx_memory_usage (vctx_stack_default( 2) v) (vctx_stack_default( 3) v))
      (vctx_stack_default( 4) v) (vctx_stack_default( 5) v)
  | Misc RETURN ->
    m v.vctx_memory_usage (vctx_stack_default( 0) v) (vctx_stack_default( 1) v)
  | Log _ -> m v.vctx_memory_usage (vctx_stack_default( 0) v) (vctx_stack_default( 1) v)
  | _ -> v.vctx_memory_usage
  ))

(*val predict_gas : inst -> variable_ctx -> constant_ctx -> integer*)
let predict_gas i v c1:Nat_big_num.num=  
 (c v.vctx_memory_usage (new_memory_consumption i v) (vctx_next_instruction_default v c1)
...
let check_resources v c1 s i:bool=  
 ((match inst_stack_numbers i with
  | (consumed, produced) ->
    ((((List.length s) + produced) - consumed) <= 1024) &&
    ( Nat_big_num.less_equal(predict_gas i v c1) v.vctx_gas)
  ))


(*val subtract_gas : integer -> instruction_result -> instruction_result*)
let subtract_gas consumption orig:instruction_result=  
 ((match orig with
  | InstructionContinue v ->
    InstructionContinue ({ v with vctx_gas = (Nat_big_num.sub v.vctx_gas consumption) })
  | InstructionAnnotationFailure -> InstructionAnnotationFailure
  | InstructionToEnvironment( act, v, opt) ->
    InstructionToEnvironment( act, ({v with vctx_gas = (Nat_big_num.sub v.vctx_gas consumption)}), opt)
  ))
...
let instruction_sem v c1 inst1:instruction_result=  
 (subtract_gas (predict_gas inst1 v c1)
  ((match inst1 with
  ...
  | Info GASPRICE -> stack_0_1_op v c1 v.vctx_block.block_gasprice
  ...
  | Misc CALLCODE -> callcode v c1
  | Misc SUICIDE -> suicide v c1
  | Misc DELEGATECALL -> delegatecall v c1
...
(* Since our program struct contains a list of annotations for each program position, *)
(* I have a function that checks all annotations at a particular program position: *)
(*val check_annotations : variable_ctx -> constant_ctx -> bool*)
let check_annotations v c1:bool=  
 (let annots : annotation list = (c1.cctx_program.program_annotation v.vctx_pc) in
  List.for_all (fun annot -> annot (build_aenv v c1)) annots)


(*val next_state: (instruction_result -> unit) -> constant_ctx -> instruction_result -> instruction_result*)
let next_state stopper c1 pr:instruction_result=  
 ((match pr with
  | InstructionToEnvironment( _, _, _) ->
    let () = (stopper pr) in pr
  | InstructionAnnotationFailure -> let () = (stopper pr) in pr
  | InstructionContinue v ->
     if not (check_annotations v c1) then InstructionAnnotationFailure else
     (match vctx_next_instruction v c1 with
      | None -> InstructionToEnvironment( (ContractFail [ShouldNotHappen]), v, None)
      | Some i ->
        if check_resources v c1 v.vctx_stack i then
          instruction_sem v c1 i
        else
          InstructionToEnvironment( (ContractFail
              ((match inst_stack_numbers i with
               | (consumed, produced) ->
                 
                  List.rev_append (List.rev (if ((((List.length v.vctx_stack) + produced) - consumed) <= 1024) then [] else [TooLongStack])) (if Nat_big_num.less_equal (predict_gas i v c1) v.vctx_gas then [] else [OutOfGas])
               )
              )),
              v, None)
     )
...
let rec program_sem stopper c1 fuel pr:instruction_result= 
  (
  if(fuel =  0) then pr else
    (let fuel_pred0 =(Nat_num.nat_monus fuel ( 1)) in
     program_sem stopper c1 fuel_pred0 (next_state stopper c1 pr)))
...
let build_cctx a:constant_ctx=  
 ({ cctx_program = (a.account_code); cctx_this = (a.account_address) })
...
(*indreln [build_vctx_called : account_state -> call_env -> variable_ctx -> bool]
  vctx_called : forall bal a env origin ext block gas existence.
    (Instance_Basic_classes_Ord_Num_natural.>=) (word256ToNatural block.block_number) (( Instance_Num_NumMult_Num_natural.* )2463 1000) && (* only the newest version of EVM matters *)
...
vctx_returned:
   forall a_code v_pc new_bal a_bal a_addr a_storage v_stack v_memory v_memory_usage
          ...
          v_origin v_ext_program v_ext_program' v_block v_gas v_gas' mem_start mem_size
          r rest whichever v_ex v_ex' v_touched v_logs.
