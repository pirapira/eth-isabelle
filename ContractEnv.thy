...
text {* The machine word library is one of the biggest reasons for choosing Isabelle/HOL
for this development. *}
text {* The Ethereum Virtual Machine depends on 8-bit bytes and 256-bit machine words. *}
...
record aenv =
  ...
  aenv_balance_at_call :: "address \<Rightarrow> uint" -- {* The balance of all accounts at the time of the
  invocation *}
  aenv_this :: address -- {* The address of this contract under verification. *}
  aenv_origin :: address -- {* The original external account that started the whole transaction. *}

text {* The @{term aenv_balance} keeps track of the balance of all accounts because the contract
under verification sends some Eth to other accounts.  To capture the effect of this, I chose to
keep track of the balances of the other contracts.  *}

text {* @{term aenv_storage_at_call} and @{term aenv_balance_at_call} keep the states at the
time of the contract invocation.  These are used for rollback after a failure.
The contract under verification might fail
when it jumps to a wrong destination, or it runs out of gas. *}

text {* @{term aenv_origin} might be the same but might be different from @{term aenv_caller}.
...
text {* I'm going to add more fields in the @{typ aenv} record in the near future. *}
