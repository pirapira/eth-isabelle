...
| "returnable_result (InstructionToEnvironment (ContractCall _) _ _ _ _) = True"
| "returnable_result (InstructionToEnvironment (ContractDelegateCall _) _ _ _ _) = False"
| "returnable_result (InstructionToEnvironment (ContractCreate _) _ _ _ _) = True"
| "returnable_result (InstructionToEnvironment ContractSuicide _ _ _ _) = False"
...
| "returnable_result (InstructionToEnvironment (ContractReturn _) _ _ _ _) = False"
...
| "returnable_from_delegate_call (InstructionToEnvironment (ContractCall _) _ _ _ _) = False"
| "returnable_from_delegate_call (InstructionToEnvironment (ContractDelegateCall _) _ _ _ _) = True"
| "returnable_from_delegate_call (InstructionToEnvironment (ContractCreate _) _ _ _ _) = False"
| "returnable_from_delegate_call (InstructionToEnvironment ContractSuicide _ _ _ _) = False"
| "returnable_from_delegate_call (InstructionToEnvironment (ContractFail _) _ _ _ _) = False"
-- {* because we are not modeling nested calls here, the effect of the nested calls are modeled in
      account\_state\_return\_change *}
| "returnable_from_delegate_call (InstructionToEnvironment (ContractReturn _) _ _ _ _) = False"
...
where
  ...
  "(* Under a constant environment built from the old account state, *)
   ...
   program_sem k cctx steps (InstructionContinue old_vctx)
      = InstructionToEnvironment act bal touched v opt_v \<Longrightarrow>

   (* and if the account state is updated from the program's result, *)
   account_state_going_out
     = update_account_state old_account act bal v opt_v \<Longrightarrow>

   (* the contract makes a move and udates the account state. *)
   contract_turn (old_account, old_vctx)
      (account_state_going_out, Execution (InstructionToEnvironment act bal touched v opt_v))"
...
"star (one_round I)
      (a, Execution (InstructionToEnvironment (ContractFail x) bal touched v v_opt))
      (b, c) \<Longrightarrow> b = a \<and> c = Execution (InstructionToEnvironment (ContractFail x) bal touched v v_opt)"
...
"star (one_round I)
      (a, Execution (InstructionToEnvironment (ContractReturn data) bal touched v v_opt))
      (b, c) \<Longrightarrow> b = a \<and> c = Execution (InstructionToEnvironment (ContractReturn data) bal touched v v_opt)"
apply(drule star_case; simp)
apply(simp add: one_round.simps add: environment_turn.simps)
done
...
"star (one_round I)
      (a, Execution (InstructionToEnvironment ContractSuicide bal touched v v_opt))
      (b, c) \<Longrightarrow> b = a \<and> c = Execution (InstructionToEnvironment ContractSuicide bal touched v v_opt)"
apply(drule star_case; simp)
apply(simp add: one_round.simps add: environment_turn.simps)
done
...
"I state \<Longrightarrow>
 no_assertion_failure_post I (state, Execution (InstructionToEnvironment (ContractFail x) bal touched v v_opt))"
