...
let test_one_case j : testResult =
  ...
  | InstructionContinue _ ->
     ...
     TestFailure
  | InstructionToEnvironment (ContractCall carg, st, bal, touched, logs, v, pushed_opt) ->
     let () = Printf.eprintf "We are not looking whatever happens after the contract calls\n" in
     TestSkipped
  | InstructionToEnvironment (ContractDelegateCall carg, st, bal, touched, logs, v, pushed_opt) ->
     let () = Printf.eprintf "We are not looking whatever happens after the contract calls\n" in
     TestSkipped
  | InstructionToEnvironment (ContractCreate carg, st, bal, touched, logs, v, pushed_opt) ->
     let () = Printf.eprintf "We are not looking whatever happens after the contract creates" in
     TestSkipped
  | InstructionToEnvironment (ContractFail _, st, bal, touched, logs, v, pushed_opt) ->
     begin
  ...
  | InstructionToEnvironment (ContractSuicide, st, bal, touched, logs, v, pushed_opt) ->
     begin
       match test_case.callcreates, test_case.gas, test_case.logs, test_case.out, test_case.post with
  ...
  | InstructionToEnvironment (ContractReturn retval, st, bal, touched, logs, v, None) ->
     begin
       match test_case.callcreates, test_case.gas, test_case.logs, test_case.out, test_case.post with
       | spec_created, Some spec_gas, Some spec_logs, Some spec_out, Some spec_post ->
  ...
  | InstructionToEnvironment (ContractReturn retval, st, bal, touched, logs, v, Some _) ->
